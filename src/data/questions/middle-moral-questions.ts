import type { Question } from './types';

/**
 * 初中道法题库 — 3个知识点 × 30题 = 90道题
 */
export const middleMoralQuestions: Question[] = [
  // ==================== 心理健康 ====================
  {
    id: 'q-mmor-1-1', type: 'choice', difficulty: 'basic',
    stem: '认识自我的途径有自我评价和他人评价，下列属于通过他人评价认识自己的是（    ）。',
    options: ['A. 我做事很有毅力', 'B. 老师说我有责任心', 'C. 我认为自己很聪明', 'D. 我觉得自己需要改进'],
    answer: 'B',
    explanation: '他人评价是通过他人的态度和评价来认识自己。"老师说我有责任心"属于他人评价。',
    tags: ['心理健康', '认识自我'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-2', type: 'choice', difficulty: 'basic',
    stem: '下列属于基本情绪的是（    ）。',
    options: ['A. 喜、怒、哀、惧', 'B. 喜、怒、哀、乐', 'C. 焦虑、抑郁、愤怒、悲伤', 'D. 内疚、害羞、兴奋、恐惧'],
    answer: 'A',
    explanation: '基本情绪有四种：喜、怒、哀、惧。其他情绪都是由这四种基本情绪派生出来的。',
    tags: ['心理健康', '情绪'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-3', type: 'choice', difficulty: 'basic',
    stem: '当我们遇到压力和挫折时，可以通过（    ）来调节情绪。',
    options: ['A. 暴饮暴食', 'B. 合理宣泄，如运动、倾诉', 'C. 封闭自己', 'D. 逃避问题'],
    answer: 'B',
    explanation: '合理宣泄是调节情绪的有效方法，如运动、倾诉等。暴饮暴食、封闭自己、逃避问题都是不健康的方式。',
    tags: ['心理健康', '情绪调节'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-4', type: 'choice', difficulty: 'basic',
    stem: '友谊的特质是（    ）。',
    options: ['A. 单向的、短暂的', 'B. 亲密、平等、双向、心灵相遇', 'C. 只有利益关系', 'D. 不需要尊重和信任'],
    answer: 'B',
    explanation: '友谊的特质：亲密关系、平等双向、心灵相遇。真正的友谊需要尊重和信任，是双向的。',
    tags: ['心理健康', '友谊'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-5', type: 'fill-blank', difficulty: 'basic',
    stem: '情绪调节的方法包括：改变认知评价、转移注意、合理宣泄和______。',
    answer: '放松训练',
    explanation: '情绪调节四方法：改变认知评价、转移注意、合理宣泄（运动、倾诉）、放松训练。',
    tags: ['心理健康', '情绪调节'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-6', type: 'fill-blank', difficulty: 'basic',
    stem: '认识自己的途径包括自我评价和______。',
    answer: '他人评价',
    explanation: '认识自我的途径：自我评价（自我观察）和他人评价（要重视但客观分析）。',
    tags: ['心理健康', '认识自我'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-7', type: 'true-false', difficulty: 'basic',
    stem: '正确认识自己可以促进自我发展，促进与他人的交往。',
    answer: 'true',
    explanation: '正确认识自己的重要性：可以促进自我发展，促进与他人的交往。',
    tags: ['心理健康', '认识自我'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-8', type: 'true-false', difficulty: 'basic',
    stem: '生活中难免有挫折，挫折是生命成长的一部分。',
    answer: 'true',
    explanation: '挫折具有普遍性，生活难免有挫折，挫折是生命成长的一部分，可以磨砺意志。',
    tags: ['心理健康', '挫折'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-9', type: 'true-false', difficulty: 'basic',
    stem: '对待挫折的态度不同，结果就不同。',
    answer: 'true',
    explanation: '面对挫折的态度不同，结果就不同。积极面对挫折可以增长智慧，战胜困难；消极面对则会一蹶不振。',
    tags: ['心理健康', '挫折'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-10', type: 'choice', difficulty: 'intermediate',
    stem: '小明考试考砸了，下列做法体现正确面对挫折的是（    ）。',
    options: ['A. 自暴自弃，不再学习', 'B. 分析原因，改进方法', 'C. 怨天尤人，怪题太难', 'D. 封闭自己，不和同学交流'],
    answer: 'B',
    explanation: '面对挫折的正确做法：正确对待，分析原因，寻求方法改进。自暴自弃、怨天尤人都是错误的。',
    tags: ['心理健康', '挫折应对'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-11', type: 'choice', difficulty: 'intermediate',
    stem: '呵护友谊的正确做法是（    ）。',
    options: ['A. 事事替朋友做决定', 'B. 用心关怀、尊重对方、正确处理冲突', 'C. 朋友有错不管不问', 'D. 只在需要帮助时才找朋友'],
    answer: 'B',
    explanation: '呵护友谊需要：用心关怀、尊重对方、正确处理冲突、正确对待交友中的伤害。',
    tags: ['心理健康', '友谊'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-12', type: 'choice', difficulty: 'intermediate',
    stem: '关于网上交往，下列说法正确的是（    ）。',
    options: ['A. 网上交往都是虚假的，不可信', 'B. 网上交往要增强自我保护意识', 'C. 可以随意把网友转化为现实朋友', 'D. 网上交往不需要遵守规则'],
    answer: 'B',
    explanation: '网上交往的特点：虚拟、平等、自主。要增强自我保护意识，慎重把网友转化为现实中的朋友。',
    tags: ['心理健康', '网上交往'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-13', type: 'choice', difficulty: 'intermediate',
    stem: '友谊需要讲究原则。下列做法体现友谊讲原则的是（    ）。',
    options: ['A. 朋友犯错帮忙隐瞒', 'B. 朋友抄作业视而不见', 'C. 劝朋友遵守校规校纪', 'D. 朋友借钱不还也不追究'],
    answer: 'C',
    explanation: '友谊要讲原则，不能没有底线。劝朋友遵守校规校纪是真正为朋友好，体现了友谊的原则性。',
    tags: ['心理健康', '友谊原则'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-14', type: 'fill-blank', difficulty: 'intermediate',
    stem: '应对挫折的方法：增强生命韧性，发掘自身力量；培养面对困难的勇气和坚强意志；学会借助外力，______。',
    answer: '寻求帮助',
    explanation: '应对挫折的三方面：①发掘自身力量；②培养勇气和意志；③借助外力，寻求帮助。',
    tags: ['心理健康', '挫折应对'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-15', type: 'fill-blank', difficulty: 'intermediate',
    stem: '我们要重视他人的态度与评价，但也要______，既不盲从，也不忽视。',
    answer: '客观冷静分析',
    explanation: '对待他人评价的态度：要重视，但客观冷静分析，既不盲从也不忽视，用理性的心态面对。',
    tags: ['心理健康', '他人评价'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-16', type: 'true-false', difficulty: 'intermediate',
    stem: '朋友之间应该无条件支持和认同对方的一切行为。',
    answer: 'false',
    explanation: '友谊要讲原则，朋友犯错应该善意指出，不能无条件认同对方的一切行为。',
    tags: ['心理健康', '友谊原则'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-17', type: 'true-false', difficulty: 'intermediate',
    stem: '情绪会相互感染，我们要学会以恰当的方式表达情绪。',
    answer: 'true',
    explanation: '情绪具有感染性，一个人的情绪会影响周围的人，所以要学会以恰当方式表达情绪。',
    tags: ['心理健康', '情绪表达'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-18', type: 'true-false', difficulty: 'intermediate',
    stem: '我们只需要自我评价就能正确认识自己，不需要他人评价。',
    answer: 'false',
    explanation: '认识自我需要自我评价和他人评价相结合，他人评价是认识自己的一面镜子。',
    tags: ['心理健康', '认识自我'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-19', type: 'choice', difficulty: 'advanced',
    stem: '下列对挫折的理解，正确的是（    ）。',
    options: ['A. 挫折只会给人带来损失和痛苦', 'B. 聪明的人不会遇到挫折', 'C. 挫折具有两面性，可以磨砺人也可能摧毁人', 'D. 只有未成年人才会遭遇挫折'],
    answer: 'C',
    explanation: '挫折具有两面性：一方面可能阻碍人，带来损失和痛苦；另一方面可以磨砺意志，增长智慧。',
    tags: ['心理健康', '挫折两面性'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-20', type: 'choice', difficulty: 'advanced',
    stem: '小明和小华是好朋友。小明发现小华在考试中作弊，正确的做法是（    ）。',
    options: ['A. 为了友谊帮助隐瞒', 'B. 向老师举报让朋友受罚', 'C. 私下劝朋友诚信考试并说明危害', 'D. 当作没看见'],
    answer: 'C',
    explanation: '友谊要讲原则。私下劝朋友诚信考试，既维护了友谊的底线，也真正帮助了朋友。',
    tags: ['心理健康', '友谊原则'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-21', type: 'choice', difficulty: 'advanced',
    stem: '面对网上交往，我们应该（    ）。',
    options: ['A. 完全拒绝网络交往，远离网络', 'B. 沉迷网络社交，放弃现实社交', 'C. 理性辨别、慎重选择，增强自我保护意识', 'D. 接受所有网友邀请，扩大社交圈'],
    answer: 'C',
    explanation: '网上交往要理性辨别、慎重选择，增强自我保护意识，把网友转化为现实朋友要更加慎重。',
    tags: ['心理健康', '网上交往'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-22', type: 'choice', difficulty: 'advanced',
    stem: '调节情绪的合理宣泄法包括（    ）。',
    options: ['A. 在课堂上大喊大叫', 'B. 在合适的地方放声歌唱或运动', 'C. 对同学发脾气', 'D. 暴饮暴食'],
    answer: 'B',
    explanation: '合理宣泄要注意场合和方式，在合适的地方放声歌唱、运动等都是合理的宣泄方式。',
    tags: ['心理健康', '情绪调节'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-23', type: 'fill-blank', difficulty: 'advanced',
    stem: '情绪管理的意义：良好的情绪能激励我们______，不良情绪可能让我们止步不前。',
    answer: '克服困难（或努力向上、积极行动等）',
    explanation: '情绪的作用非常神奇，影响我们的观念和行动。良好的情绪激励人克服困难、努力向上。',
    tags: ['心理健康', '情绪作用'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-24', type: 'fill-blank', difficulty: 'advanced',
    stem: '正确对待他人评价的方法：用心聆听、______、平静拒绝。',
    answer: '勇于面对',
    explanation: '对待他人评价的方法：用心聆听、勇于面对（客观面对）、平静拒绝（不合理的要求）。',
    tags: ['心理健康', '他人评价'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-25', type: 'true-false', difficulty: 'advanced',
    stem: '面对挫折，不同态度会产生不同结果。拥有积极态度的人能够发现自己的潜力并战胜挫折。',
    answer: 'true',
    explanation: '面对挫折的态度不同结果不同。积极面对挫折能激发潜能、增长智慧；消极面对则会被挫折击倒。',
    tags: ['心理健康', '挫折'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-26', type: 'true-false', difficulty: 'advanced',
    stem: '网上交往是虚拟的，不需要对自己的言行负责。',
    answer: 'false',
    explanation: '虽然网上交往具有虚拟性，但网络也是现实世界的延伸，也要对自己的言行负责，遵守网络道德和法律。',
    tags: ['心理健康', '网上交往'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-27', type: 'true-false', difficulty: 'advanced',
    stem: '自我评价和他人评价结合起来，才能客观全面地认识自己。',
    answer: 'true',
    explanation: '认识自我需要多种途径结合：自我评价和他人评价结合起来，才能更客观、全面地认识自己。',
    tags: ['心理健康', '认识自我'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-28', type: 'choice', difficulty: 'intermediate',
    stem: '下列关于情绪表达的说法，正确的是（    ）。',
    options: ['A. 情绪是个人的事，想怎么表达都行', 'B. 情绪会相互感染，要以恰当方式表达', 'C. 表达情绪不需要考虑他人感受', 'D. 积极情绪不需要表达'],
    answer: 'B',
    explanation: '情绪会相互感染，一个人的情绪会影响周围人，所以要学会以恰当方式表达情绪。',
    tags: ['心理健康', '情绪表达'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-29', type: 'fill-blank', difficulty: 'intermediate',
    stem: '调节情绪的"改变认知评价"法是指从______的角度看待问题，从而改变情绪。',
    answer: '不同（或积极）',
    explanation: '改变认知评价：从不同的角度看问题，换一个角度思考，就会有不同的感受和情绪。',
    tags: ['心理健康', '情绪调节'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-1-30', type: 'fill-blank', difficulty: 'advanced',
    stem: '友谊的力量体现在：______（写出一点即可）。',
    answer: '朋友见证了我们一起走过的成长历程（或丰富生活经验、更深刻地体悟生命美好等）',
    explanation: '友谊的力量：①朋友对一个人的影响很大，见证了成长历程；②丰富了生活经验；③让我们更深刻地体悟生命的美好。',
    tags: ['心理健康', '友谊'],
    knowledgePointIds: ['m-mor-001'], subject: '道法', grade: 'middle',
  },

  // ==================== 法律常识 ====================
  {
    id: 'q-mmor-2-1', type: 'choice', difficulty: 'basic',
    stem: '法律区别于道德等行为规范的最主要特征是（    ）。',
    options: ['A. 由国家制定或认可', 'B. 由国家强制力保证实施', 'C. 对全体社会成员具有普遍约束力', 'D. 规定了权利和义务'],
    answer: 'B',
    explanation: '法律的特征：由国家制定或认可；由国家强制力保证实施（最主要特征）；对全体社会成员具有普遍约束力。',
    tags: ['法律常识', '法律特征'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-2', type: 'choice', difficulty: 'basic',
    stem: '宪法是国家的根本法，具有（    ）的法律效力。',
    options: ['A. 与普通法律相同', 'B. 最低', 'C. 最高', 'D. 不确定'],
    answer: 'C',
    explanation: '宪法具有最高的法律效力，是其他法律的立法基础和立法依据。',
    tags: ['法律常识', '宪法'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-3', type: 'choice', difficulty: 'basic',
    stem: '犯罪的三个基本特征不包括（    ）。',
    options: ['A. 严重社会危害性', 'B. 刑事违法性', 'C. 应受刑罚处罚性', 'D. 社会危害性轻微'],
    answer: 'D',
    explanation: '犯罪的三个特征：严重社会危害性（最本质特征）、刑事违法性、应受刑罚处罚性。"轻微"不属于犯罪。',
    tags: ['法律常识', '犯罪特征'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-4', type: 'choice', difficulty: 'basic',
    stem: '未成年人受到的保护体系中，不包括（    ）。',
    options: ['A. 家庭保护', 'B. 学校保护', 'C. 社会保护', 'D. 亲戚保护'],
    answer: 'D',
    explanation: '未成年人六大保护：家庭保护、学校保护、社会保护、网络保护、政府保护、司法保护。',
    tags: ['法律常识', '未成年人保护'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-5', type: 'fill-blank', difficulty: 'basic',
    stem: '宪法规定国家生活中最______、最重要的问题。',
    answer: '根本',
    explanation: '从内容看，宪法规定国家生活中最根本、最重要的问题，如国家性质、根本制度等。',
    tags: ['法律常识', '宪法'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-6', type: 'fill-blank', difficulty: 'basic',
    stem: '违法行为分为民事违法、行政违法和______。',
    answer: '刑事违法（犯罪）',
    explanation: '违法行为按性质分：民事违法、行政违法、刑事违法（犯罪是最严重的违法行为）。',
    tags: ['法律常识', '违法分类'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-7', type: 'true-false', difficulty: 'basic',
    stem: '法律是最刚性的社会规则，不违法是人们行为的底线。',
    answer: 'true',
    explanation: '法律是最刚性的社会规则。不违法是人们的行为底线，违法必受法律制裁。',
    tags: ['法律常识', '法律'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-8', type: 'true-false', difficulty: 'basic',
    stem: '宪法是治国安邦的总章程，是公民权利的保障书。',
    answer: 'true',
    explanation: '宪法的作用：①是治国安邦的总章程；②是公民权利的保障书；③规范国家权力运行。',
    tags: ['法律常识', '宪法'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-9', type: 'true-false', difficulty: 'basic',
    stem: '违法就是犯罪，犯罪就是违法。',
    answer: 'false',
    explanation: '违法不一定犯罪（一般违法与犯罪有区别），犯罪一定违法。犯罪是最严重的违法行为。',
    tags: ['法律常识', '违法与犯罪'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-10', type: 'choice', difficulty: 'intermediate',
    stem: '下列属于犯罪行为的是（    ）。',
    options: ['A. 上课迟到', 'B. 拾到他人手机不归还', 'C. 持刀抢劫致人重伤', 'D. 考试作弊'],
    answer: 'C',
    explanation: '持刀抢劫致人重伤具有严重社会危害性、刑事违法性、应受刑罚处罚性，属于犯罪行为。',
    tags: ['法律常识', '犯罪'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-11', type: 'choice', difficulty: 'intermediate',
    stem: '遇到侵害，最正规、最权威的手段是（    ）。',
    options: ['A. 协商', 'B. 调解', 'C. 诉讼（打官司）', 'D. 寻求法律援助'],
    answer: 'C',
    explanation: '诉讼（打官司）是处理纠纷和应对侵害最正规、最权威的手段，是维护合法权益的最后屏障。',
    tags: ['法律常识', '依法维权'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-12', type: 'choice', difficulty: 'intermediate',
    stem: '学校周边商店向学生出售香烟，这违反了（    ）。',
    options: ['A. 家庭保护', 'B. 学校保护', 'C. 社会保护', 'D. 司法保护'],
    answer: 'C',
    explanation: '社会保护是指社会各主体为未成年人健康成长创造良好环境。商店向学生售烟违反了社会保护。',
    tags: ['法律常识', '社会保护'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-13', type: 'choice', difficulty: 'intermediate',
    stem: '下列关于宪法与其他法律关系的说法，正确的是（    ）。',
    options: ['A. 宪法和其他法律地位平等', 'B. 宪法是其他法律的立法基础和立法依据', 'C. 其他法律是宪法的立法基础', 'D. 宪法包含了其他法律的全部内容'],
    answer: 'B',
    explanation: '宪法具有最高法律效力，是其他法律的立法基础和立法依据，其他法律不得与宪法相抵触。',
    tags: ['法律常识', '宪法效力'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-14', type: 'fill-blank', difficulty: 'intermediate',
    stem: '宪法从______、______、______三个方面具有最高的法律地位。（写出两点即可）',
    answer: '内容、法律效力、制定和修改程序（任答两点）',
    explanation: '宪法最高地位：①内容规定最根本最重要问题；②具有最高法律效力；③制定和修改程序更严格。',
    tags: ['法律常识', '宪法地位'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-15', type: 'fill-blank', difficulty: 'intermediate',
    stem: '刑罚分为主刑和附加刑。主刑包括管制、拘役、有期徒刑、无期徒刑和______。',
    answer: '死刑',
    explanation: '主刑：管制、拘役、有期徒刑、无期徒刑、死刑（主刑只能独立适用）。',
    tags: ['法律常识', '刑罚'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-16', type: 'true-false', difficulty: 'intermediate',
    stem: '一般违法和犯罪都要承担法律责任，都会受到刑罚处罚。',
    answer: 'false',
    explanation: '一般违法要承担民事责任或行政处罚；犯罪才会受到刑罚处罚。',
    tags: ['法律常识', '违法与犯罪'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-17', type: 'true-false', difficulty: 'intermediate',
    stem: '法律的作用包括规范作用（规定权利和义务）和保护作用（制裁违法犯罪）。',
    answer: 'true',
    explanation: '法律的作用：规范作用（规定权利和义务、规范行为）；保护作用（制裁违法犯罪、保护公民权益）。',
    tags: ['法律常识', '法律作用'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-18', type: 'true-false', difficulty: 'intermediate',
    stem: '人民法院在审判时，考虑到被告是未成年人而依法从轻处罚，体现了社会保护。',
    answer: 'false',
    explanation: '人民法院对未成年人依法从轻处罚，体现的是司法保护，不是社会保护。',
    tags: ['法律常识', '司法保护'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-19', type: 'choice', difficulty: 'advanced',
    stem: '区分一般违法和犯罪的关键是看（    ）。',
    options: ['A. 违法者年龄大小', 'B. 是否具有严重社会危害性', 'C. 违法次数多少', 'D. 是否被警察抓住'],
    answer: 'B',
    explanation: '犯罪的最本质特征是严重社会危害性。区分一般违法和犯罪的关键是看社会危害程度是否严重。',
    tags: ['法律常识', '违法与犯罪'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-20', type: 'choice', difficulty: 'advanced',
    stem: '小明在商场买到过期食品，依法维权的正确途径是（    ）。',
    options: ['A. 纠集朋友去商场闹事', 'B. 忍气吞声，自认倒霉', 'C. 与商家协商或向消费者协会投诉', 'D. 破坏商场设施以泄愤'],
    answer: 'C',
    explanation: '依法维权途径：协商→调解→仲裁→诉讼。C选项"协商或向消协投诉"是合法的维权方式。',
    tags: ['法律常识', '依法维权'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-21', type: 'choice', difficulty: 'advanced',
    stem: '关于宪法监督，下列说法正确的是（    ）。',
    options: ['A. 普通公民无权监督宪法的实施', 'B. 全国人大及其常委会行使宪法监督权', 'C. 宪法监督是政府的职责', 'D. 宪法实施不需要监督'],
    answer: 'B',
    explanation: '全国人大及其常委会行使监督宪法实施的职权。公民也有权监督宪法的实施。',
    tags: ['法律常识', '宪法监督'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-22', type: 'choice', difficulty: 'advanced',
    stem: '预防青少年违法犯罪，下列说法错误的是（    ）。',
    options: ['A. 要增强法治观念，遵纪守法', 'B. 有了不良行为要及时矫治', 'C. 偶尔去网吧没关系，不会犯罪', 'D. 认清犯罪危害，远离犯罪'],
    answer: 'C',
    explanation: '预防犯罪要从杜绝不良行为做起。"偶尔去网吧没关系"是错误的认识，不良行为可能发展为违法犯罪。',
    tags: ['法律常识', '预防犯罪'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-23', type: 'fill-blank', difficulty: 'advanced',
    stem: '诉讼的类型包括民事诉讼、行政诉讼和______。',
    answer: '刑事诉讼',
    explanation: '诉讼的三种类型：民事诉讼（民告民）、行政诉讼（民告官）、刑事诉讼（追究犯罪）。',
    tags: ['法律常识', '诉讼类型'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-24', type: 'fill-blank', difficulty: 'advanced',
    stem: '当我们合法权益受到侵害时，可以通过法律服务机构来维护合法权益，如______、______等。（写出两个）',
    answer: '律师事务所、公证处（或法律援助中心等）',
    explanation: '法律服务机构：律师事务所、公证处、法律援助中心等，可提供法律咨询和帮助。',
    tags: ['法律常识', '法律救济'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-25', type: 'true-false', difficulty: 'advanced',
    stem: '法律面前人人平等，任何人不论职务高低、功劳大小，只要触犯法律都要承担法律责任。',
    answer: 'true',
    explanation: '法律面前人人平等是法律的基本原则。任何组织和个人都不得有超越法律的特权，违法犯罪都要承担法律责任。',
    tags: ['法律常识', '法律平等'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-26', type: 'true-false', difficulty: 'advanced',
    stem: '未成年人受到法律的特殊保护，所以未成年人违法不需要承担法律责任。',
    answer: 'false',
    explanation: '未成年人受到法律的特殊保护，但违法同样要承担法律责任，只是根据年龄和情节可能从轻或减轻处罚。',
    tags: ['法律常识', '未成年人保护'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-27', type: 'true-false', difficulty: 'advanced',
    stem: '我国的根本政治制度体现了人民当家作主，是公民权利的保障。',
    answer: 'true',
    explanation: '人民代表大会制度是根本政治制度，保证了人民当家作主，保障了公民的权利。',
    tags: ['法律常识', '政治制度'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-28', type: 'choice', difficulty: 'intermediate',
    stem: '下列关于法律特征的说法，正确的是（    ）。',
    options: ['A. 法律对部分人有约束力', 'B. 法律靠道德约束', 'C. 法律由国家强制力保证实施', 'D. 法律不需要制定'],
    answer: 'C',
    explanation: '法律特征：①由国家制定或认可；②由国家强制力保证实施（最主要）；③对全体成员有普遍约束力。',
    tags: ['法律常识', '法律特征'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-29', type: 'fill-blank', difficulty: 'intermediate',
    stem: '犯罪的严重社会危害性是犯罪的______特征。',
    answer: '最本质',
    explanation: '犯罪的三个特征中，严重社会危害性是最本质的特征，刑事违法性是法律标志，刑罚处罚性是法律后果。',
    tags: ['法律常识', '犯罪特征'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-2-30', type: 'fill-blank', difficulty: 'advanced',
    stem: '我们要增强法治意识，依法办事，青少年应该成为法治的忠实崇尚者、自觉遵守者和______。',
    answer: '坚定捍卫者',
    explanation: '青少年要增强法治意识，成为法治的忠实崇尚者、自觉遵守者、坚定捍卫者。',
    tags: ['法律常识', '法治意识'],
    knowledgePointIds: ['m-mor-002'], subject: '道法', grade: 'middle',
  },

  // ==================== 国情国策 ====================
  {
    id: 'q-mmor-3-1', type: 'choice', difficulty: 'basic',
    stem: '我国现阶段的基本国情是（    ）。',
    options: ['A. 我国已进入发达国家行列', 'B. 我国仍处于社会主义初级阶段', 'C. 我国已实现共同富裕', 'D. 我国已进入社会主义高级阶段'],
    answer: 'B',
    explanation: '我国仍处于并将长期处于社会主义初级阶段，这是我国的基本国情。',
    tags: ['国情国策', '基本国情'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-2', type: 'choice', difficulty: 'basic',
    stem: '我国基本经济制度中处于主体地位的是（    ）。',
    options: ['A. 私营经济', 'B. 外资经济', 'C. 公有制经济', 'D. 混合所有制经济'],
    answer: 'C',
    explanation: '公有制为主体、多种所有制经济共同发展。公有制经济在国民经济中处于主体地位。',
    tags: ['国情国策', '基本经济制度'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-3', type: 'choice', difficulty: 'basic',
    stem: '我国的根本政治制度是（    ）。',
    options: ['A. 民族区域自治制度', 'B. 基层群众自治制度', 'C. 人民代表大会制度', 'D. 中国共产党领导的多党合作制度'],
    answer: 'C',
    explanation: '人民代表大会制度是我国的根本政治制度。ABD都是基本政治制度。',
    tags: ['国情国策', '政治制度'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-4', type: 'choice', difficulty: 'basic',
    stem: '发展的根本目的是（    ）。',
    options: ['A. 提高国际地位', 'B. 增进民生福祉', 'C. 增强军事实力', 'D. 扩大领土'],
    answer: 'B',
    explanation: '发展的根本目的是增进民生福祉，让发展成果惠及全体人民。',
    tags: ['国情国策', '发展思想'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-5', type: 'fill-blank', difficulty: 'basic',
    stem: '创新的定位是引领发展的______。',
    answer: '第一动力',
    explanation: '创新是引领发展的第一动力，是民族进步的灵魂。',
    tags: ['国情国策', '创新'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-6', type: 'fill-blank', difficulty: 'basic',
    stem: '我国的基本政治制度包括中国共产党领导的多党合作和政治协商制度、民族区域自治制度和______。',
    answer: '基层群众自治制度',
    explanation: '三个基本政治制度：多党合作和政治协商制度、民族区域自治制度、基层群众自治制度。',
    tags: ['国情国策', '政治制度'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-7', type: 'true-false', difficulty: 'basic',
    stem: '国有经济是国民经济的主导力量。',
    answer: 'true',
    explanation: '国有经济是国民经济的主导力量，对于发挥社会主义制度的优越性具有关键作用。',
    tags: ['国情国策', '经济制度'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-8', type: 'true-false', difficulty: 'basic',
    stem: '人民代表大会是国家权力机关，全国人民代表大会是最高国家权力机关。',
    answer: 'true',
    explanation: '人民代表大会是国家权力机关，全国人大是最高国家权力机关。',
    tags: ['国情国策', '人民代表大会'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-9', type: 'true-false', difficulty: 'basic',
    stem: '"绿水青山就是金山银山"体现了绿色发展理念。',
    answer: 'true',
    explanation: '"绿水青山就是金山银山"体现了绿色发展理念和生态文明建设的要求。',
    tags: ['国情国策', '生态文明'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-10', type: 'choice', difficulty: 'intermediate',
    stem: '实施科教兴国的目标是（    ）。',
    options: ['A. 用先进的科技武装头脑', 'B. 把教育摆在优先发展地位，科技与教育相结合', 'C. 用科技取代教育', 'D. 只发展教育'],
    answer: 'B',
    explanation: '科教兴国战略：把教育摆在优先发展地位，科技与教育相结合，增强科技实力，提高劳动者素质。',
    tags: ['国情国策', '科教兴国'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-11', type: 'choice', difficulty: 'intermediate',
    stem: '我国的强国之路是（    ）。',
    options: ['A. 改革开放', 'B. 科技创新', 'C. 教育发展', 'D. 依法治国'],
    answer: 'A',
    explanation: '改革开放是强国之路，是决定当代中国命运的关键抉择，是当代中国最鲜明的特色。',
    tags: ['国情国策', '改革开放'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-12', type: 'choice', difficulty: 'intermediate',
    stem: '中国梦的基本内涵是（    ）。',
    options: ['A. 全面建成小康社会', 'B. 实现中华民族伟大复兴（国家富强、民族振兴、人民幸福）', 'C. 成为世界霸主', 'D. 实现同步富裕'],
    answer: 'B',
    explanation: '中国梦的基本内涵：实现中华民族伟大复兴，具体包括国家富强、民族振兴、人民幸福。',
    tags: ['国情国策', '中国梦'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-13', type: 'choice', difficulty: 'intermediate',
    stem: '关于可持续发展，下列说法正确的是（    ）。',
    options: ['A. 经济发展最重要，环境可以暂时不管', 'B. 坚持节约资源和保护环境的基本国策', 'C. 只要经济发展，环境问题自然解决', 'D. 环保与发展无关'],
    answer: 'B',
    explanation: '可持续发展要求经济发展与环境保护相协调，坚持节约资源和保护环境的基本国策。',
    tags: ['国情国策', '可持续发展'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-14', type: 'fill-blank', difficulty: 'intermediate',
    stem: '我国坚持节约资源和保护环境的______，走绿色发展道路。',
    answer: '基本国策',
    explanation: '节约资源和保护环境是我国的基本国策，要贯彻绿色发展理念，建设美丽中国。',
    tags: ['国情国策', '生态文明'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-15', type: 'fill-blank', difficulty: 'intermediate',
    stem: '我国的分配制度是按劳分配为主体、______并存。',
    answer: '多种分配方式',
    explanation: '分配制度：按劳分配为主体、多种分配方式并存。',
    tags: ['国情国策', '分配制度'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-16', type: 'true-false', difficulty: 'intermediate',
    stem: '创新是民族进步的灵魂，是一个国家兴旺发达的不竭源泉。',
    answer: 'true',
    explanation: '创新是引领发展的第一动力，是民族进步的灵魂，是一个国家兴旺发达的不竭源泉。',
    tags: ['国情国策', '创新'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-17', type: 'true-false', difficulty: 'intermediate',
    stem: '改革开放是决定当代中国命运的关键抉择，是当代中国最鲜明的特色。',
    answer: 'true',
    explanation: '改革开放是强国之路，是决定当代中国命运的关键抉择，是当代中国最鲜明的特色。',
    tags: ['国情国策', '改革开放'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-18', type: 'true-false', difficulty: 'intermediate',
    stem: '教育是民族振兴、社会进步的基石，是提高国民素质的根本途径。',
    answer: 'true',
    explanation: '教育是民族振兴和社会进步的基石，是提高国民素质、促进人的全面发展的根本途径。',
    tags: ['国情国策', '教育'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-19', type: 'choice', difficulty: 'advanced',
    stem: '关于共同富裕，下列理解正确的是（    ）。',
    options: ['A. 共同富裕就是同时富裕、同步富裕', 'B. 共同富裕是一个过程，鼓励一部分人先富起来', 'C. 共同富裕就是平均分配', 'D. 共同富裕只适用于城市'],
    answer: 'B',
    explanation: '共同富裕不是同时富裕、同步富裕，而是一个过程。鼓励一部分人、一部分地区先富起来，带动和帮助后富，逐步实现共同富裕。',
    tags: ['国情国策', '共同富裕'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-20', type: 'choice', difficulty: 'advanced',
    stem: '"两个一百年"奋斗目标中，第二个百年目标是（    ）。',
    options: ['A. 全面建成小康社会', 'B. 实现共产主义', 'C. 全面建成社会主义现代化强国', 'D. 达到中等发达国家水平'],
    answer: 'C',
    explanation: '第一个百年目标（建党百年）：全面建成小康社会（已实现）；第二个百年目标（新中国成立百年）：全面建成社会主义现代化强国。',
    tags: ['国情国策', '奋斗目标'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-21', type: 'choice', difficulty: 'advanced',
    stem: '下列关于基本经济制度的说法，错误的是（    ）。',
    options: ['A. 公有制经济包括国有经济、集体经济', 'B. 非公有制经济是社会主义市场经济的重要组成部分', 'C. 国有经济在国民经济中起主导作用', 'D. 私营经济是国民经济的主导力量'],
    answer: 'D',
    explanation: '国有经济是国民经济的主导力量，不是私营经济。D选项说法错误。',
    tags: ['国情国策', '经济制度'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-22', type: 'choice', difficulty: 'advanced',
    stem: '关于民族区域自治制度，下列说法正确的是（    ）。',
    options: ['A. 少数民族可以完全独立', 'B. 在国家统一领导下，在少数民族聚居地区实行区域自治', 'C. 所有民族都可以设立自治区', 'D. 民族区域自治制度是根本政治制度'],
    answer: 'B',
    explanation: '民族区域自治制度是在国家统一领导下，在少数民族聚居的地方实行区域自治，不是完全独立。',
    tags: ['国情国策', '民族区域自治'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-23', type: 'fill-blank', difficulty: 'advanced',
    stem: '我国的战略体系包括科教兴国战略、人才强国战略和______。',
    answer: '创新驱动发展战略',
    explanation: '三大发展战略：科教兴国战略、人才强国战略、创新驱动发展战略。',
    tags: ['国情国策', '发展战略'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-24', type: 'fill-blank', difficulty: 'advanced',
    stem: '建设生态文明，要坚持______的发展道路，建设资源节约型、环境友好型社会。',
    answer: '绿色（或可持续发展）',
    explanation: '坚持绿色发展道路，走生产发展、生活富裕、生态良好的文明发展道路。',
    tags: ['国情国策', '生态文明'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-25', type: 'true-false', difficulty: 'advanced',
    stem: '创新的目的是增进人类福祉，让生活更美好。',
    answer: 'true',
    explanation: '创新的目的是增进人类福祉，让生活更美好。创新让生活更便捷、更美好。',
    tags: ['国情国策', '创新'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-26', type: 'true-false', difficulty: 'advanced',
    stem: '人民代表大会制度是我国的基本政治制度。',
    answer: 'false',
    explanation: '人民代表大会制度是我国的根本政治制度。民族区域自治制度、多党合作制度、基层群众自治制度才是基本政治制度。',
    tags: ['国情国策', '政治制度'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-27', type: 'true-false', difficulty: 'advanced',
    stem: '国有经济和公有制经济在国民经济中都起主导作用。',
    answer: 'false',
    explanation: '国有经济起主导作用，公有制经济处于主体地位。两者地位不同。',
    tags: ['国情国策', '经济制度'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-28', type: 'choice', difficulty: 'intermediate',
    stem: '关于生态文明建设，下列做法值得提倡的是（    ）。',
    options: ['A. 乱砍滥伐森林', 'B. 垃圾分类，绿色出行', 'C. 工业废水直接排入河流', 'D. 大量使用一次性筷子'],
    answer: 'B',
    explanation: '垃圾分类、绿色出行是践行生态文明理念的正确做法。ACD都是破坏环境的行为。',
    tags: ['国情国策', '生态文明'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-29', type: 'fill-blank', difficulty: 'intermediate',
    stem: '中国共产党领导是中国特色社会主义______的特征。',
    answer: '最本质',
    explanation: '中国共产党领导是中国特色社会主义最本质的特征，是中国特色社会主义制度的最大优势。',
    tags: ['国情国策', '党的领导'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mmor-3-30', type: 'fill-blank', difficulty: 'advanced',
    stem: '进入新时代，我国社会的主要矛盾已经转化为人民日益增长的美好生活需要和______之间的矛盾。',
    answer: '不平衡不充分的发展',
    explanation: '新时代主要矛盾：人民日益增长的美好生活需要和不平衡不充分的发展之间的矛盾。',
    tags: ['国情国策', '主要矛盾'],
    knowledgePointIds: ['m-mor-003'], subject: '道法', grade: 'middle',
  },

  // ==================== 权利与义务 (m-mor-004) ====================
  {
    id: 'q-mm-004-1', type: 'choice', difficulty: 'basic',
    stem: '下列关于"权利与义务"的说法，正确的是（    ）。',
    options: ['A. 它是${subject}学习的基础', 'B. 它不重要', 'C. 它只在高年级学习', 'D. 它不需要练习'],
    answer: 'A',
    explanation: '权利与义务是道法学习的基础内容，需要认真掌握。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-2', type: 'fill-blank', difficulty: 'basic',
    stem: '学习"权利与义务"要______。',
    answer: '认真听讲、勤加练习',
    explanation: '学习权利与义务需要认真听讲、勤加练习。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-3', type: 'true-false', difficulty: 'basic',
    stem: '"权利与义务"是初中道法学习的重要内容。',
    answer: 'true',
    explanation: '权利与义务是初中道法学习的重要内容。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-4', type: 'choice', difficulty: 'basic',
    stem: '下列例子中，与"权利与义务"相关的是（    ）。',
    options: ['A. 例一', 'B. 例二', 'C. 例三', 'D. 例四'],
    answer: 'A',
    explanation: 'A选项与权利与义务相关。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-5', type: 'fill-blank', difficulty: 'basic',
    stem: '"权利与义务"的关键是______。',
    answer: '理解和运用',
    explanation: '学习权利与义务的关键是理解和运用。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-6', type: 'true-false', difficulty: 'basic',
    stem: '学习"权利与义务"只需要死记硬背。',
    answer: 'false',
    explanation: '学习权利与义务需要理解和运用，不能只靠死记硬背。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-7', type: 'choice', difficulty: 'basic',
    stem: '关于"权利与义务"的学习方法，下列说法正确的是（    ）。',
    options: ['A. 多读多练', 'B. 不练习', 'C. 只听课', 'D. 不复习'],
    answer: 'A',
    explanation: '学习权利与义务要多读多练。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-8', type: 'choice', difficulty: 'intermediate',
    stem: '运用"权利与义务"知识，下列解释最准确的是（    ）。',
    options: ['A. 例一', 'B. 例二', 'C. 例三', 'D. 例四'],
    answer: 'B',
    explanation: 'B选项运用了权利与义务的知识，解释准确。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-9', type: 'fill-blank', difficulty: 'intermediate',
    stem: '请写出一个体现"权利与义务"的例子：______。',
    answer: '开放性答案',
    explanation: '请运用权利与义务的知识写一个例子。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-10', type: 'true-false', difficulty: 'intermediate',
    stem: '"权利与义务"只在考试中用，与日常生活无关。',
    answer: 'false',
    explanation: '权利与义务与日常生活密切相关。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-11', type: 'choice', difficulty: 'intermediate',
    stem: '下列关于"权利与义务"的理解，正确的是（    ）。',
    options: ['A. 需要理解运用', 'B. 不需要理解', 'C. 只在高年级用', 'D. 不重要'],
    answer: 'A',
    explanation: '权利与义务需要理解运用。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-12', type: 'fill-blank', difficulty: 'intermediate',
    stem: '"权利与义务"与道法学习的关系是______。',
    answer: '密切相关',
    explanation: '权利与义务与道法学习密切相关。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-13', type: 'true-false', difficulty: 'intermediate',
    stem: '学习"权利与义务"需要联系生活实际。',
    answer: 'true',
    explanation: '联系生活实际有助于理解权利与义务。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-14', type: 'choice', difficulty: 'intermediate',
    stem: '下列现象中，"权利与义务"解释正确的是（    ）。',
    options: ['A. 例一', 'B. 例二', 'C. 例三', 'D. 例四'],
    answer: 'A',
    explanation: 'A选项的权利与义务解释正确。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-15', type: 'choice', difficulty: 'advanced',
    stem: '阅读下列材料，分析其中"权利与义务"的原理，正确的是（    ）。',
    options: ['A. 原理一', 'B. 原理二', 'C. 原理三', 'D. 原理四'],
    answer: 'C',
    explanation: 'C选项正确分析了权利与义务的原理。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-16', type: 'fill-blank', difficulty: 'advanced',
    stem: '请运用"权利与义务"分析下面的问题：______。',
    answer: '开放性答案',
    explanation: '请运用权利与义务知识分析问题。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-17', type: 'true-false', difficulty: 'advanced',
    stem: '"权利与义务"在不同情境中的应用可能不同。',
    answer: 'true',
    explanation: '权利与义务在不同情境中的应用可能不同。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-18', type: 'choice', difficulty: 'advanced',
    stem: '下列关于"权利与义务"的理解，最深刻的是（    ）。',
    options: ['A. 表面理解', 'B. 浅层理解', 'C. 深层理解', 'D. 错误理解'],
    answer: 'C',
    explanation: '深层理解权利与义务的本质。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-19', type: 'fill-blank', difficulty: 'advanced',
    stem: '请用一段话描述"权利与义务"的特点：______。',
    answer: '开放性答案',
    explanation: '请描述权利与义务的特点。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-20', type: 'true-false', difficulty: 'advanced',
    stem: '掌握"权利与义务"需要长期积累。',
    answer: 'true',
    explanation: '权利与义务的学习需要长期积累。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-21', type: 'choice', difficulty: 'advanced',
    stem: '下列关于"权利与义务"的应用，最恰当的是（    ）。',
    options: ['A. 应用一', 'B. 应用二', 'C. 应用三', 'D. 应用四'],
    answer: 'B',
    explanation: 'B选项的权利与义务应用最恰当。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },
  {
    id: 'q-mm-004-22', type: 'fill-blank', difficulty: 'advanced',
    stem: '请设计一个验证"权利与义务"的实验或活动：______。',
    answer: '开放性答案',
    explanation: '请设计实验或活动验证权利与义务。',
    tags: ['权利与义务'],
    knowledgePointIds: ['m-mor-004'], subject: '道法', grade: 'middle',
  },

];
