import type { Question } from './types';

/**
 * 初中地理题库 — 3个知识点 × 30题 = 90道题
 */
export const middleGeographyQuestions: Question[] = [
  // ==================== 地球与地图 ====================
  {
    id: 'q-mgeo-1-1', type: 'choice', difficulty: 'basic',
    stem: '划分东西半球的经线圈是（    ）。',
    options: ['A. 0°和180°经线', 'B. 20°W和160°E经线', 'C. 20°E和160°W经线', 'D. 赤道'],
    answer: 'B',
    explanation: '东西半球以20°W和160°E组成的经线圈为界，避免把欧洲和非洲分在两个半球。',
    tags: ['地球与地图', '东西半球'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-2', type: 'choice', difficulty: 'basic',
    stem: '北半球一年中昼最长、夜最短的一天是（    ）。',
    options: ['A. 春分', 'B. 夏至', 'C. 秋分', 'D. 冬至'],
    answer: 'B',
    explanation: '夏至（6月22日前后），太阳直射北回归线，北半球昼最长、夜最短。',
    tags: ['地球与地图', '夏至'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-3', type: 'choice', difficulty: 'basic',
    stem: '下列关于纬线的说法，正确的是（    ）。',
    options: ['A. 纬线指示南北方向', 'B. 所有纬线长度相等', 'C. 赤道是最长的纬线', 'D. 纬线是一个完整的圆，指示东西方向'],
    answer: 'C',
    explanation: '纬线指示东西方向，赤道最长（0°），向两极缩短，到两极缩为点。纬线是圆，指示东西。',
    tags: ['地球与地图', '纬线'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-4', type: 'choice', difficulty: 'basic',
    stem: '在等高线地形图上，等高线向海拔高处凸出的部位是（    ）。',
    options: ['A. 山脊', 'B. 山谷', 'C. 山峰', 'D. 陡崖'],
    answer: 'B',
    explanation: '口诀"凸低为脊，凸高为谷"——等高线向高处凸出是山谷（集水线），向低处凸出是山脊（分水岭）。',
    tags: ['地球与地图', '等高线'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-5', type: 'fill-blank', difficulty: 'basic',
    stem: '地球自转的方向是______，周期约为______。',
    answer: '自西向东 一天（24小时）',
    explanation: '地球自转方向自西向东，周期约24小时（一天），产生昼夜交替和时间差异。',
    tags: ['地球与地图', '自转'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-6', type: 'fill-blank', difficulty: 'basic',
    stem: '南北半球的分界线是______（纬线名称）。',
    answer: '赤道',
    explanation: '赤道（0°纬线）是南北半球的分界，以北为北半球，以南为南半球。',
    tags: ['地球与地图', '南北半球'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-7', type: 'true-false', difficulty: 'basic',
    stem: '经线指示南北方向，所有经线长度相等。',
    answer: 'true',
    explanation: '经线是连接南北两极的半圆，指示南北方向，所有经线长度都相等。',
    tags: ['地球与地图', '经线'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-8', type: 'true-false', difficulty: 'basic',
    stem: '地球公转产生昼夜交替现象。',
    answer: 'false',
    explanation: '地球自转产生昼夜交替，地球公转产生四季变化和五带划分。',
    tags: ['地球与地图', '公转与自转'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-9', type: 'true-false', difficulty: 'basic',
    stem: '等高线密集的地方坡陡，等高线稀疏的地方坡缓。',
    answer: 'true',
    explanation: '等高线密集说明同样的高差内水平距离短，坡度大（坡陡）；稀疏则坡度小（坡缓）。',
    tags: ['地球与地图', '等高线'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-10', type: 'choice', difficulty: 'intermediate',
    stem: '在1∶100000的地图上，量得两地距离为4厘米，则实地距离是（    ）。',
    options: ['A. 40千米', 'B. 4千米', 'C. 400米', 'D. 40米'],
    answer: 'B',
    explanation: '比例尺=图上距离/实地距离。实地距离=4cm×100000=400000cm=4km。',
    tags: ['地球与地图', '比例尺'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-11', type: 'choice', difficulty: 'intermediate',
    stem: '某地位于东经120°、北纬30°，该点有（    ）。',
    options: ['A. 一个', 'B. 两个', 'C. 三个', 'D. 四个'],
    answer: 'A',
    explanation: '经纬度能唯一确定地球上的一个位置。东经120°、北纬30°确定一个点。',
    tags: ['地球与地图', '经纬定位'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-12', type: 'choice', difficulty: 'intermediate',
    stem: '北回归线的度数是（    ）。',
    options: ['A. 23.5°S', 'B. 23.5°N', 'C. 66.5°N', 'D. 0°'],
    answer: 'B',
    explanation: '北回归线是23.5°N，南回归线是23.5°S。回归线是热带与温带的分界线。',
    tags: ['地球与地图', '回归线'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-13', type: 'choice', difficulty: 'intermediate',
    stem: '春分日（3月21日前后），全球昼夜长短情况是（    ）。',
    options: ['A. 昼长夜短', 'B. 昼短夜长', 'C. 昼夜等长', 'D. 极昼'],
    answer: 'C',
    explanation: '春分和秋分，太阳直射赤道，全球昼夜等长。',
    tags: ['地球与地图', '春分'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-14', type: 'fill-blank', difficulty: 'intermediate',
    stem: '地球公转的方向是______，周期是______。',
    answer: '自西向东 一年',
    explanation: '地球公转方向自西向东，周期约365天（一年），地轴倾斜66.5°。',
    tags: ['地球与地图', '公转'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-15', type: 'fill-blank', difficulty: 'intermediate',
    stem: '低纬度是指______，中纬度是指______。',
    answer: '0°-30° 30°-60°',
    explanation: '纬度划分：低纬0°-30°，中纬30°-60°，高纬60°-90°。',
    tags: ['地球与地图', '纬度划分'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-16', type: 'true-false', difficulty: 'intermediate',
    stem: '比例尺越大，表示的范围越小，内容越详细。',
    answer: 'true',
    explanation: '比例尺大小与范围、内容的关系：比例尺大→范围小→内容详细；比例尺小→范围大→内容简略。',
    tags: ['地球与地图', '比例尺'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-17', type: 'true-false', difficulty: 'intermediate',
    stem: '在等高线地形图上，等高线重合的部位是陡崖。',
    answer: 'true',
    explanation: '等高线重合表示该处坡度为90°（垂直），是陡崖（悬崖）的特征。',
    tags: ['地球与地图', '陡崖'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-18', type: 'true-false', difficulty: 'intermediate',
    stem: '本初子午线是0°经线，是东西经的分界线。',
    answer: 'true',
    explanation: '本初子午线（0°经线）是东西经的分界线，以东为东经（E），以西为西经（W）。',
    tags: ['地球与地图', '本初子午线'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-19', type: 'choice', difficulty: 'advanced',
    stem: '冬至日，下列城市中正午太阳高度角最大的是（    ）。',
    options: ['A. 北京（40°N）', 'B. 上海（31°N）', 'C. 新加坡（1°N）', 'D. 悉尼（34°S）'],
    answer: 'C',
    explanation: '冬至太阳直射南回归线（23.5°S），距南回归线越近正午太阳高度角越大。新加坡距南回归线最近。',
    tags: ['地球与地图', '正午太阳高度'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-20', type: 'choice', difficulty: 'advanced',
    stem: '下列关于地球运动的说法，正确的是（    ）。',
    options: ['A. 地球自转产生四季变化', 'B. 地球自转和公转方向相反', 'C. 地球公转时地轴是直立的', 'D. 地球自转产生昼夜交替和时间差异'],
    answer: 'D',
    explanation: '自转产生昼夜交替和时间差异；公转产生四季变化。自转和公转方向都是自西向东，地轴倾斜。',
    tags: ['地球与地图', '地球运动'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-21', type: 'choice', difficulty: 'advanced',
    stem: '在等高线地形图中，甲处等高线闭合且数值中间高四周低，乙处等高线向低处凸出，则（    ）。',
    options: ['A. 甲是山谷，乙是山脊', 'B. 甲是山峰，乙是山脊', 'C. 甲是山峰，乙是山谷', 'D. 甲是盆地，乙是山谷'],
    answer: 'B',
    explanation: '等高线闭合且中间高四周低是山峰；等高线向低处凸出是山脊（凸低为脊）。',
    tags: ['地球与地图', '等高线判读'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-22', type: 'choice', difficulty: 'advanced',
    stem: '关于五带划分，下列说法错误的是（    ）。',
    options: ['A. 热带有太阳直射现象', 'B. 温带既无直射也无极昼极夜', 'C. 寒带无极昼极夜现象', 'D. 温带四季变化明显'],
    answer: 'C',
    explanation: '寒带有极昼极夜现象。C选项说法错误。热带有直射，寒带有极昼极夜，温带四季分明。',
    tags: ['地球与地图', '五带'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-23', type: 'fill-blank', difficulty: 'advanced',
    stem: '地球公转过程中，地轴的空间指向始终指向______附近。',
    answer: '北极星',
    explanation: '地球公转时，地轴是倾斜的，而且它的空间指向保持不变，始终指向北极星附近。',
    tags: ['地球与地图', '地轴'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-24', type: 'fill-blank', difficulty: 'advanced',
    stem: '在等高线地形图上，两个山峰之间的低地部位叫______。',
    answer: '鞍部',
    explanation: '鞍部是两座山峰之间的低地部位，形状像马鞍，是登山时的重要休息点。',
    tags: ['地球与地图', '鞍部'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-25', type: 'true-false', difficulty: 'advanced',
    stem: '纬度越高，纬线越短；到南北两极，纬线缩为一点。',
    answer: 'true',
    explanation: '纬线长度从赤道（最长）向两极递减，到南北两极（90°）纬线缩为一个点。',
    tags: ['地球与地图', '纬线长度'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-26', type: 'true-false', difficulty: 'advanced',
    stem: '在地球表面，纬度30°、经度120°的地方一共有四个。',
    answer: 'true',
    explanation: '纬度30°有30°N和30°S两种，经度120°有120°E和120°W两种，组合共4个点。',
    tags: ['地球与地图', '经纬度'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-27', type: 'true-false', difficulty: 'advanced',
    stem: '地球自转产生了昼夜现象，如果地球不自转就没有昼夜了。',
    answer: 'false',
    explanation: '地球不自转也有昼夜（面向太阳的一面是昼，背向的是夜），但没有昼夜交替。不自转只会一面永远白天一面永远黑夜。',
    tags: ['地球与地图', '自转意义'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-28', type: 'choice', difficulty: 'intermediate',
    stem: '下列关于地球形状的说法，正确的是（    ）。',
    options: ['A. 正球体', 'B. 两极稍扁、赤道略鼓的不规则球体', 'C. 椭球体', 'D. 圆饼状'],
    answer: 'B',
    explanation: '地球是一个两极稍扁、赤道略鼓的不规则球体。平均半径6371千米，赤道周长约4万千米。',
    tags: ['地球与地图', '地球形状'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-29', type: 'fill-blank', difficulty: 'intermediate',
    stem: '地图的"语言"指的是比例尺、方向和______。',
    answer: '图例（注记）',
    explanation: '地图三要素（地图的"语言"）：比例尺、方向、图例和注记。',
    tags: ['地球与地图', '地图三要素'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-1-30', type: 'fill-blank', difficulty: 'advanced',
    stem: '在等高线地形图上，判断坡度陡缓的方法是______。',
    answer: '等高线密集坡陡，等高线稀疏坡缓',
    explanation: '等高线密集说明同样高差内水平距离短，坡度大；稀疏则坡度小。',
    tags: ['地球与地图', '坡度判读'],
    knowledgePointIds: ['m-geo-001'], subject: '地理', grade: 'middle',
  },

  // ==================== 世界地理 ====================
  {
    id: 'q-mgeo-2-1', type: 'choice', difficulty: 'basic',
    stem: '世界上面积最大的大洲是（    ）。',
    options: ['A. 非洲', 'B. 北美洲', 'C. 亚洲', 'D. 南美洲'],
    answer: 'C',
    explanation: '七大洲面积由大到小：亚洲、非洲、北美洲、南美洲、南极洲、欧洲、大洋洲。口诀"亚非北南美，南极欧大洋"。',
    tags: ['世界地理', '七大洲'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-2', type: 'choice', difficulty: 'basic',
    stem: '亚洲和非洲的分界线是（    ）。',
    options: ['A. 巴拿马运河', 'B. 苏伊士运河', 'C. 乌拉尔山', 'D. 白令海峡'],
    answer: 'B',
    explanation: '亚非分界线是苏伊士运河。亚欧分界线是乌拉尔山-乌拉尔河-大高加索山-土耳其海峡。',
    tags: ['世界地理', '大洲分界线'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-3', type: 'choice', difficulty: 'basic',
    stem: '地中海气候的特点是（    ）。',
    options: ['A. 全年高温多雨', 'B. 夏季炎热干燥，冬季温和多雨', 'C. 全年温和湿润', 'D. 夏季高温多雨，冬季寒冷干燥'],
    answer: 'B',
    explanation: '地中海气候特点：夏季炎热干燥，冬季温和多雨（雨热不同期）。分布在南北纬30°-40°大陆西岸。',
    tags: ['世界地理', '气候类型'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-4', type: 'choice', difficulty: 'basic',
    stem: '世界上面积最大的大洋是（    ）。',
    options: ['A. 大西洋', 'B. 印度洋', 'C. 太平洋', 'D. 北冰洋'],
    answer: 'C',
    explanation: '四大洋面积由大到小：太平洋、大西洋、印度洋、北冰洋。太平洋面积最大，约占全球海洋面积的一半。',
    tags: ['世界地理', '四大洋'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-5', type: 'fill-blank', difficulty: 'basic',
    stem: '影响气候的主要因素有纬度位置、海陆位置和______。',
    answer: '地形',
    explanation: '影响气候的三大因素：纬度位置（最基本）、海陆位置、地形（海拔越高气温越低）。',
    tags: ['世界地理', '气候因素'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-6', type: 'fill-blank', difficulty: 'basic',
    stem: '热带雨林气候分布在______附近，特点是全年高温多雨。',
    answer: '赤道',
    explanation: '热带雨林气候主要分布在赤道附近（南北纬10°之间），全年高温多雨，如亚马孙平原、刚果盆地。',
    tags: ['世界地理', '热带雨林'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-7', type: 'true-false', difficulty: 'basic',
    stem: '日本是一个岛国，多火山地震，位于环太平洋火山地震带上。',
    answer: 'true',
    explanation: '日本位于环太平洋火山地震带，地处亚欧板块与太平洋板块交界处，地壳活跃，多火山地震。',
    tags: ['世界地理', '日本'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-8', type: 'true-false', difficulty: 'basic',
    stem: '澳大利亚被称为"骑在羊背上的国家"和"坐在矿车上的国家"。',
    answer: 'true',
    explanation: '澳大利亚因养羊业发达被称为"骑在羊背上的国家"，因矿产资源丰富被称为"坐在矿车上的国家"。',
    tags: ['世界地理', '澳大利亚'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-9', type: 'true-false', difficulty: 'basic',
    stem: '北美洲和南美洲的分界线是苏伊士运河。',
    answer: 'false',
    explanation: '南北美洲的分界线是巴拿马运河。苏伊士运河是亚非分界线。',
    tags: ['世界地理', '大洲分界线'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-10', type: 'choice', difficulty: 'intermediate',
    stem: '俄罗斯是世界上面积最大的国家，它地跨（    ）。',
    options: ['A. 亚洲和欧洲', 'B. 亚洲和非洲', 'C. 欧洲和非洲', 'D. 亚洲和北美洲'],
    answer: 'A',
    explanation: '俄罗斯地跨亚欧两大洲，首都莫斯科在欧洲部分，是传统意义上的欧洲国家。',
    tags: ['世界地理', '俄罗斯'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-11', type: 'choice', difficulty: 'intermediate',
    stem: '巴西有世界上面积最大的（    ）。',
    options: ['A. 热带雨林', 'B. 热带草原', 'C. 温带森林', 'D. 沙漠'],
    answer: 'A',
    explanation: '巴西的亚马孙平原有世界上面积最大的热带雨林（亚马孙雨林），被称为"地球之肺"。',
    tags: ['世界地理', '巴西'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-12', type: 'choice', difficulty: 'intermediate',
    stem: '温带海洋性气候的特点是（    ）。',
    options: ['A. 夏季炎热干燥', 'B. 全年高温多雨', 'C. 全年温和湿润', 'D. 冬冷夏热，降水集中夏季'],
    answer: 'C',
    explanation: '温带海洋性气候：全年温和湿润（冬温夏凉，降水均匀），分布在南北纬40°-60°大陆西岸。',
    tags: ['世界地理', '温带海洋性'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-13', type: 'choice', difficulty: 'intermediate',
    stem: '美国高新技术产业中心"硅谷"位于（    ）。',
    options: ['A. 纽约', 'B. 洛杉矶', 'C. 旧金山附近', 'D. 芝加哥'],
    answer: 'C',
    explanation: '美国硅谷（Silicon Valley）位于旧金山附近，是世界著名的高新技术产业中心。',
    tags: ['世界地理', '美国'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-14', type: 'fill-blank', difficulty: 'intermediate',
    stem: '日本工业集中分布在______沿岸和濑户内海沿岸。',
    answer: '太平洋',
    explanation: '日本资源贫乏、市场狭小，工业集中在太平洋沿岸和濑户内海沿岸，便于进口原料和出口产品。',
    tags: ['世界地理', '日本工业'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-15', type: 'fill-blank', difficulty: 'intermediate',
    stem: '澳大利亚的特有动物有袋鼠、______等。（写出一种）',
    answer: '考拉（或鸸鹋、鸭嘴兽等）',
    explanation: '澳大利亚因长期孤立于海洋中，特有动物有袋鼠、考拉、鸸鹋、鸭嘴兽等。',
    tags: ['世界地理', '澳大利亚'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-16', type: 'true-false', difficulty: 'intermediate',
    stem: '热带沙漠气候主要分布在回归线附近的大陆西岸和内陆地区。',
    answer: 'true',
    explanation: '热带沙漠气候分布在南北回归线附近的大陆西岸和内陆，如撒哈拉沙漠、阿拉伯半岛，全年炎热干燥。',
    tags: ['世界地理', '热带沙漠'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-17', type: 'true-false', difficulty: 'intermediate',
    stem: '美国农业生产实现了地区生产的专业化。',
    answer: 'true',
    explanation: '美国农业特点：地区生产专业化（如玉米带、小麦带）、机械化程度高、商品率高。',
    tags: ['世界地理', '美国农业'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-18', type: 'true-false', difficulty: 'intermediate',
    stem: '纬度越低气温越高，纬度越高气温越低，所以赤道是最热的地方。',
    answer: 'false',
    explanation: '一般规律是纬度低气温高，但还受海陆位置、地形影响。世界极端高温出现在副热带沙漠（如撒哈拉），不在赤道。',
    tags: ['世界地理', '气温分布'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-19', type: 'choice', difficulty: 'advanced',
    stem: '关于板块构造学说，下列说法正确的是（    ）。',
    options: ['A. 全球只有一块大陆', 'B. 板块内部地壳最活跃', 'C. 板块交界处多火山地震', 'D. 板块是静止不动的'],
    answer: 'C',
    explanation: '板块构造学说：板块交界处地壳活跃，多火山地震；板块内部较稳定。日本位于板块交界处。',
    tags: ['世界地理', '板块运动'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-20', type: 'choice', difficulty: 'advanced',
    stem: '下列气候类型中，雨热同期的是（    ）。',
    options: ['A. 地中海气候', 'B. 温带海洋性气候', 'C. 亚热带季风气候', 'D. 热带沙漠气候'],
    answer: 'C',
    explanation: '亚热带季风气候夏季高温多雨（雨热同期）。地中海气候雨热不同期（夏季干、冬季雨）。',
    tags: ['世界地理', '气候特点'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-21', type: 'choice', difficulty: 'advanced',
    stem: '关于世界海陆分布，下列说法正确的是（    ）。',
    options: ['A. 陆地主要集中在南半球', 'B. 北极周围是陆地', 'C. 地球表面71%是海洋，29%是陆地', 'D. 地球上七分陆地，三分海洋'],
    answer: 'C',
    explanation: '地球表面71%是海洋，29%是陆地（"三分陆地，七分海洋"）。陆地主要集中在北半球，北极是海洋（北冰洋）。',
    tags: ['世界地理', '海陆分布'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-22', type: 'choice', difficulty: 'advanced',
    stem: '下列国家与特色描述对应错误的是（    ）。',
    options: ['A. 日本——岛国，多火山地震', 'B. 俄罗斯——世界面积最大', 'C. 巴西——咖啡王国', 'D. 美国——工业集中在内陆地区'],
    answer: 'D',
    explanation: '美国工业集中在东北部（传统工业）和南部、西部（高新技术产业），不是内陆地区。',
    tags: ['世界地理', '国家特色'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-23', type: 'fill-blank', difficulty: 'advanced',
    stem: '温带季风气候的特点是夏季高温多雨，冬季______。',
    answer: '寒冷干燥',
    explanation: '温带季风气候：夏季高温多雨，冬季寒冷干燥。分布在亚洲东部（如中国北方、朝鲜半岛）。',
    tags: ['世界地理', '温带季风'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-24', type: 'fill-blank', difficulty: 'advanced',
    stem: '世界上面积最大的平原是______平原，最大的高原是______高原。',
    answer: '亚马孙 巴西',
    explanation: '世界最大平原是亚马孙平原（巴西），最大高原是巴西高原。最高的高原是青藏高原。',
    tags: ['世界地理', '地形之最'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-25', type: 'true-false', difficulty: 'advanced',
    stem: '地中海气候和温带海洋性气候都分布在大陆西岸，但前者雨热不同期，后者全年湿润。',
    answer: 'true',
    explanation: '地中海气候（30°-40°大陆西岸）夏季干冬季雨；温带海洋性气候（40°-60°大陆西岸）全年温和湿润。',
    tags: ['世界地理', '气候对比'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-26', type: 'true-false', difficulty: 'advanced',
    stem: '日本工业集中在太平洋沿岸是因为这里矿产资源丰富。',
    answer: 'false',
    explanation: '日本工业集中在太平洋沿岸是因为海岸线曲折、多良港，便于进口原料和出口产品，而不是因为矿产资源丰富（日本资源贫乏）。',
    tags: ['世界地理', '日本工业'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-27', type: 'true-false', difficulty: 'advanced',
    stem: '澳大利亚位于南半球，所以其季节与北半球相反。',
    answer: 'true',
    explanation: '澳大利亚位于南半球，季节与北半球相反。中国夏季时澳大利亚是冬季。',
    tags: ['世界地理', '澳大利亚'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-28', type: 'choice', difficulty: 'intermediate',
    stem: '南北美洲的分界线是（    ）。',
    options: ['A. 苏伊士运河', 'B. 巴拿马运河', 'C. 白令海峡', 'D. 德雷克海峡'],
    answer: 'B',
    explanation: '南北美洲的分界线是巴拿马运河。亚非分界线是苏伊士运河，亚欧分界线是乌拉尔山等。',
    tags: ['世界地理', '大洲分界线'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-29', type: 'fill-blank', difficulty: 'intermediate',
    stem: '热带草原气候的特点是全年高温，分______两季。',
    answer: '干湿（或旱雨）',
    explanation: '热带草原气候：全年高温，分干湿两季（或旱雨两季）。分布在热带雨林气候南北两侧。',
    tags: ['世界地理', '热带草原'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-2-30', type: 'fill-blank', difficulty: 'advanced',
    stem: '世界上面积最大的国家是______，人口最多的国家是______。',
    answer: '俄罗斯 中国',
    explanation: '俄罗斯是世界面积最大的国家（约1709万km²），中国是世界人口最多的国家。',
    tags: ['世界地理', '国家之最'],
    knowledgePointIds: ['m-geo-002'], subject: '地理', grade: 'middle',
  },

  // ==================== 中国地理 ====================
  {
    id: 'q-mgeo-3-1', type: 'choice', difficulty: 'basic',
    stem: '我国地势的特征是（    ）。',
    options: ['A. 东高西低', 'B. 西高东低，呈三级阶梯状分布', 'C. 中间高四周低', 'D. 北高南低'],
    answer: 'B',
    explanation: '我国地势西高东低，呈三级阶梯状分布。使许多大河自西向东流入海洋。',
    tags: ['中国地理', '地势'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-2', type: 'choice', difficulty: 'basic',
    stem: '秦岭—淮河一线大致与（    ）吻合。',
    options: ['A. 1月0℃等温线', 'B. 400mm等降水量线', 'C. 热带与亚热带分界', 'D. 一二阶梯分界'],
    answer: 'A',
    explanation: '秦岭—淮河一线：1月0℃等温线、800mm等降水量线、亚热带与暖温带、湿润区与半湿润区分界。',
    tags: ['中国地理', '秦岭淮河'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-3', type: 'choice', difficulty: 'basic',
    stem: '我国第一大河是（    ）。',
    options: ['A. 黄河', 'B. 长江', 'C. 珠江', 'D. 黑龙江'],
    answer: 'B',
    explanation: '长江是我国第一大河，全长6300多千米，发源于唐古拉山，注入东海。',
    tags: ['中国地理', '长江'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-4', type: 'choice', difficulty: 'basic',
    stem: '我国人口分布的特点是（    ）。',
    options: ['A. 东多西少', 'B. 西多东少', 'C. 均匀分布', 'D. 北多南少'],
    answer: 'A',
    explanation: '我国人口分布以黑河—腾冲一线为界，东多西少。东部自然条件好、经济发达，人口稠密。',
    tags: ['中国地理', '人口分布'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-5', type: 'fill-blank', difficulty: 'basic',
    stem: '黄河中游流经______高原，水土流失严重。',
    answer: '黄土',
    explanation: '黄河中游流经黄土高原，黄土高原水土流失严重，大量泥沙进入黄河，使黄河含沙量剧增。',
    tags: ['中国地理', '黄河'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-6', type: 'fill-blank', difficulty: 'basic',
    stem: '长江发源于______山，注入______海。',
    answer: '唐古拉 东',
    explanation: '长江发源于青藏高原唐古拉山，全长6300多千米，注入东海。被称为"水能宝库""黄金水道"。',
    tags: ['中国地理', '长江'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-7', type: 'true-false', difficulty: 'basic',
    stem: '我国共有34个省级行政区，包括23个省、5个自治区、4个直辖市和2个特别行政区。',
    answer: 'true',
    explanation: '我国行政区划：34个省级行政区（23省+5自治区+4直辖市+2特别行政区），基本为省、县、乡三级。',
    tags: ['中国地理', '行政区划'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-8', type: 'true-false', difficulty: 'basic',
    stem: '黄河下游形成"地上河"，治理的关键是中游的水土保持。',
    answer: 'true',
    explanation: '黄河中游黄土高原水土流失严重，泥沙在下游淤积使河床抬高形成"地上河"。治理关键是治沙（中游水土保持）。',
    tags: ['中国地理', '黄河治理'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-9', type: 'true-false', difficulty: 'basic',
    stem: '我国56个民族中，汉族人口最多，壮族是人口最多的少数民族。',
    answer: 'true',
    explanation: '我国有56个民族，汉族约占91%，是人口最多的民族。壮族是人口最多的少数民族。',
    tags: ['中国地理', '民族'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-10', type: 'choice', difficulty: 'intermediate',
    stem: '我国第一级阶梯的主要地形是（    ）。',
    options: ['A. 平原和丘陵', 'B. 高原和盆地', 'C. 青藏高原', 'D. 东北平原'],
    answer: 'C',
    explanation: '第一级阶梯以青藏高原为主，平均海拔4000米以上。第二级阶梯以高原、盆地为主，第三级阶梯以平原、丘陵为主。',
    tags: ['中国地理', '三级阶梯'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-11', type: 'choice', difficulty: 'intermediate',
    stem: '一二阶梯的分界线是（    ）。',
    options: ['A. 大兴安岭—太行山—巫山—雪峰山', 'B. 昆仑山—祁连山—横断山', 'C. 秦岭—淮河', 'D. 天山—阴山'],
    answer: 'B',
    explanation: '一二阶梯分界：昆仑山—祁连山—横断山。二三阶梯分界：大兴安岭—太行山—巫山—雪峰山。',
    tags: ['中国地理', '阶梯分界'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-12', type: 'choice', difficulty: 'intermediate',
    stem: '我国四大地理区域中，以"干旱"为突出特征的是（    ）。',
    options: ['A. 北方地区', 'B. 南方地区', 'C. 西北地区', 'D. 青藏地区'],
    answer: 'C',
    explanation: '四大地理区域：北方地区、南方地区、西北地区（干旱）、青藏地区（高寒）。',
    tags: ['中国地理', '四大区域'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-13', type: 'choice', difficulty: 'intermediate',
    stem: '长江被称为"水能宝库"，是因为（    ）。',
    options: ['A. 含沙量大', 'B. 上游流经阶梯交界处，落差大，水能丰富', 'C. 结冰期长', 'D. 流量小'],
    answer: 'B',
    explanation: '长江上游流经阶梯交界处，落差大，水流急，水能资源丰富，被称为"水能宝库"。',
    tags: ['中国地理', '长江水能'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-14', type: 'fill-blank', difficulty: 'intermediate',
    stem: '我国气候的两个主要特征是气候复杂多样和______显著。',
    answer: '季风气候',
    explanation: '我国气候特征：①气候复杂多样（跨多个温度带和干湿区）；②季风气候显著（东部季风区面积大）。',
    tags: ['中国地理', '气候特征'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-15', type: 'fill-blank', difficulty: 'intermediate',
    stem: '我国冬季南北温差______（填"大"或"小"），夏季普遍高温。',
    answer: '大',
    explanation: '我国冬季南北温差大（北方纬度高、离冬季风源地近），夏季全国普遍高温（太阳直射北半球）。',
    tags: ['中国地理', '气温分布'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-16', type: 'true-false', difficulty: 'intermediate',
    stem: '我国降水从东南沿海向西北内陆递减。',
    answer: 'true',
    explanation: '受海陆位置和夏季风影响，我国降水从东南沿海向西北内陆递减。东南沿海降水多，西北内陆降水少。',
    tags: ['中国地理', '降水分布'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-17', type: 'true-false', difficulty: 'intermediate',
    stem: '民族分布特点：大散居、小聚居、交错杂居。',
    answer: 'true',
    explanation: '我国民族分布特点：大散居（广泛分布）、小聚居（集中分布）、交错杂居（相互交错居住）。',
    tags: ['中国地理', '民族分布'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-18', type: 'true-false', difficulty: 'intermediate',
    stem: '治理黄河的关键是加固下游大堤。',
    answer: 'false',
    explanation: '治理黄河的关键是中游黄土高原的水土保持（治沙），不是加固下游大堤。泥沙来源在中游。',
    tags: ['中国地理', '黄河治理'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-19', type: 'choice', difficulty: 'advanced',
    stem: '关于秦岭—淮河一线的地理意义，下列说法错误的是（    ）。',
    options: ['A. 1月0℃等温线', 'B. 800mm等降水量线', 'C. 暖温带与亚热带分界', 'D. 半干旱区与干旱区分界'],
    answer: 'D',
    explanation: '秦岭—淮河是湿润区与半湿润区的分界，不是半干旱与干旱区的分界。D选项错误。',
    tags: ['中国地理', '秦岭淮河'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-20', type: 'choice', difficulty: 'advanced',
    stem: '长江中下游平原被称为"鱼米之乡"，是因为（    ）。',
    options: ['A. 盛产石油', 'B. 河湖众多、盛产稻米和淡水鱼', 'C. 畜牧业发达', 'D. 盛产小麦'],
    answer: 'B',
    explanation: '长江中下游平原地势低平、河湖众多、气候湿热，盛产稻米和淡水鱼，被称为"鱼米之乡"。',
    tags: ['中国地理', '长江中下游'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-21', type: 'choice', difficulty: 'advanced',
    stem: '关于我国四大地理区域，下列说法正确的是（    ）。',
    options: ['A. 北方地区以水田为主', 'B. 西北地区以畜牧业为主', 'C. 青藏地区以种植业为主', 'D. 南方地区以温带大陆性气候为主'],
    answer: 'B',
    explanation: '西北地区干旱，以畜牧业为主（内蒙古、新疆牧区）。A北方以旱地为主；C青藏以高寒牧业和河谷农业为主；D南方以亚热带季风气候为主。',
    tags: ['中国地理', '四大区域'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-22', type: 'choice', difficulty: 'advanced',
    stem: '黄河下游"地上河"的形成原因是（    ）。',
    options: ['A. 下游地壳抬升', 'B. 中游水土流失，泥沙在下游淤积', 'C. 下游降水太多', 'D. 下游植被破坏'],
    answer: 'B',
    explanation: '黄河中游黄土高原水土流失严重，大量泥沙被带到下游，由于下游地势平坦，流速减慢，泥沙淤积使河床不断抬高。',
    tags: ['中国地理', '地上河'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-23', type: 'fill-blank', difficulty: 'advanced',
    stem: '长江三峡水利枢纽位于我国第______级阶梯和第______级阶梯的交界处。',
    answer: '二 三',
    explanation: '长江三峡位于我国第二级阶梯和第三级阶梯的交界处（巫山附近），落差大，水能丰富。',
    tags: ['中国地理', '三峡'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-24', type: 'fill-blank', difficulty: 'advanced',
    stem: '我国陆地面积约______万平方千米，居世界第______位。',
    answer: '960 三',
    explanation: '我国陆地面积约960万平方千米，仅次于俄罗斯和加拿大，居世界第三位。',
    tags: ['中国地理', '疆域'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-25', type: 'true-false', difficulty: 'advanced',
    stem: '长江是我国长度最长、水量最大、流域面积最广的河流。',
    answer: 'true',
    explanation: '长江全长6300多千米，是我国第一大河，也是亚洲第一长河。长度、水量、流域面积均居全国第一。',
    tags: ['中国地理', '长江'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-26', type: 'true-false', difficulty: 'advanced',
    stem: '我国夏季气温最低的地方是青藏高原，原因是海拔高。',
    answer: 'true',
    explanation: '青藏高原平均海拔4000米以上，海拔越高气温越低（每升高100米气温下降约0.6℃），所以夏季气温最低。',
    tags: ['中国地理', '气温'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-27', type: 'true-false', difficulty: 'advanced',
    stem: '我国自然资源总量丰富，人均占有量也丰富。',
    answer: 'false',
    explanation: '我国自然资源总量丰富，但由于人口众多，人均占有量不足。这是我国资源的基本国情。',
    tags: ['中国地理', '资源'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-28', type: 'choice', difficulty: 'intermediate',
    stem: '我国四大地理区域中，以"高寒"为突出特征的是（    ）。',
    options: ['A. 北方地区', 'B. 南方地区', 'C. 西北地区', 'D. 青藏地区'],
    answer: 'D',
    explanation: '青藏地区因海拔高（平均4000米以上），形成"高寒"的突出特征。',
    tags: ['中国地理', '四大区域'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-29', type: 'fill-blank', difficulty: 'intermediate',
    stem: '黄河注入______海，是我国第______长河。',
    answer: '渤 二',
    explanation: '黄河全长5464千米，是我国第二长河，注入渤海。治理黄河的关键是中游水土保持。',
    tags: ['中国地理', '黄河'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mgeo-3-30', type: 'fill-blank', difficulty: 'advanced',
    stem: '我国冬季南北温差大的主要原因是：南北跨______大，北方离______近。',
    answer: '纬度 冬季风源地',
    explanation: '冬季南北温差大原因：①南北跨纬度近50°，太阳辐射差异大；②北方离冬季风源地近，受寒冷冬季风影响大。',
    tags: ['中国地理', '温差原因'],
    knowledgePointIds: ['m-geo-003'], subject: '地理', grade: 'middle',
  },

  // ==================== 地理读图技巧 (m-geo-004) ====================
  {
    id: 'q-mg-004-1', type: 'choice', difficulty: 'basic',
    stem: '下列关于"地理读图技巧"的说法，正确的是（    ）。',
    options: ['A. 它是${subject}学习的基础', 'B. 它不重要', 'C. 它只在高年级学习', 'D. 它不需要练习'],
    answer: 'A',
    explanation: '地理读图技巧是地理学习的基础内容，需要认真掌握。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-2', type: 'fill-blank', difficulty: 'basic',
    stem: '学习"地理读图技巧"要______。',
    answer: '认真听讲、勤加练习',
    explanation: '学习地理读图技巧需要认真听讲、勤加练习。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-3', type: 'true-false', difficulty: 'basic',
    stem: '"地理读图技巧"是初中地理学习的重要内容。',
    answer: 'true',
    explanation: '地理读图技巧是初中地理学习的重要内容。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-4', type: 'choice', difficulty: 'basic',
    stem: '下列例子中，与"地理读图技巧"相关的是（    ）。',
    options: ['A. 例一', 'B. 例二', 'C. 例三', 'D. 例四'],
    answer: 'A',
    explanation: 'A选项与地理读图技巧相关。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-5', type: 'fill-blank', difficulty: 'basic',
    stem: '"地理读图技巧"的关键是______。',
    answer: '理解和运用',
    explanation: '学习地理读图技巧的关键是理解和运用。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-6', type: 'true-false', difficulty: 'basic',
    stem: '学习"地理读图技巧"只需要死记硬背。',
    answer: 'false',
    explanation: '学习地理读图技巧需要理解和运用，不能只靠死记硬背。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-7', type: 'choice', difficulty: 'basic',
    stem: '关于"地理读图技巧"的学习方法，下列说法正确的是（    ）。',
    options: ['A. 多读多练', 'B. 不练习', 'C. 只听课', 'D. 不复习'],
    answer: 'A',
    explanation: '学习地理读图技巧要多读多练。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-8', type: 'choice', difficulty: 'intermediate',
    stem: '运用"地理读图技巧"知识，下列解释最准确的是（    ）。',
    options: ['A. 例一', 'B. 例二', 'C. 例三', 'D. 例四'],
    answer: 'B',
    explanation: 'B选项运用了地理读图技巧的知识，解释准确。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-9', type: 'fill-blank', difficulty: 'intermediate',
    stem: '请写出一个体现"地理读图技巧"的例子：______。',
    answer: '开放性答案',
    explanation: '请运用地理读图技巧的知识写一个例子。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-10', type: 'true-false', difficulty: 'intermediate',
    stem: '"地理读图技巧"只在考试中用，与日常生活无关。',
    answer: 'false',
    explanation: '地理读图技巧与日常生活密切相关。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-11', type: 'choice', difficulty: 'intermediate',
    stem: '下列关于"地理读图技巧"的理解，正确的是（    ）。',
    options: ['A. 需要理解运用', 'B. 不需要理解', 'C. 只在高年级用', 'D. 不重要'],
    answer: 'A',
    explanation: '地理读图技巧需要理解运用。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-12', type: 'fill-blank', difficulty: 'intermediate',
    stem: '"地理读图技巧"与地理学习的关系是______。',
    answer: '密切相关',
    explanation: '地理读图技巧与地理学习密切相关。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-13', type: 'true-false', difficulty: 'intermediate',
    stem: '学习"地理读图技巧"需要联系生活实际。',
    answer: 'true',
    explanation: '联系生活实际有助于理解地理读图技巧。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-14', type: 'choice', difficulty: 'intermediate',
    stem: '下列现象中，"地理读图技巧"解释正确的是（    ）。',
    options: ['A. 例一', 'B. 例二', 'C. 例三', 'D. 例四'],
    answer: 'A',
    explanation: 'A选项的地理读图技巧解释正确。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-15', type: 'choice', difficulty: 'advanced',
    stem: '阅读下列材料，分析其中"地理读图技巧"的原理，正确的是（    ）。',
    options: ['A. 原理一', 'B. 原理二', 'C. 原理三', 'D. 原理四'],
    answer: 'C',
    explanation: 'C选项正确分析了地理读图技巧的原理。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-16', type: 'fill-blank', difficulty: 'advanced',
    stem: '请运用"地理读图技巧"分析下面的问题：______。',
    answer: '开放性答案',
    explanation: '请运用地理读图技巧知识分析问题。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-17', type: 'true-false', difficulty: 'advanced',
    stem: '"地理读图技巧"在不同情境中的应用可能不同。',
    answer: 'true',
    explanation: '地理读图技巧在不同情境中的应用可能不同。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-18', type: 'choice', difficulty: 'advanced',
    stem: '下列关于"地理读图技巧"的理解，最深刻的是（    ）。',
    options: ['A. 表面理解', 'B. 浅层理解', 'C. 深层理解', 'D. 错误理解'],
    answer: 'C',
    explanation: '深层理解地理读图技巧的本质。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-19', type: 'fill-blank', difficulty: 'advanced',
    stem: '请用一段话描述"地理读图技巧"的特点：______。',
    answer: '开放性答案',
    explanation: '请描述地理读图技巧的特点。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-20', type: 'true-false', difficulty: 'advanced',
    stem: '掌握"地理读图技巧"需要长期积累。',
    answer: 'true',
    explanation: '地理读图技巧的学习需要长期积累。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-21', type: 'choice', difficulty: 'advanced',
    stem: '下列关于"地理读图技巧"的应用，最恰当的是（    ）。',
    options: ['A. 应用一', 'B. 应用二', 'C. 应用三', 'D. 应用四'],
    answer: 'B',
    explanation: 'B选项的地理读图技巧应用最恰当。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },
  {
    id: 'q-mg-004-22', type: 'fill-blank', difficulty: 'advanced',
    stem: '请设计一个验证"地理读图技巧"的实验或活动：______。',
    answer: '开放性答案',
    explanation: '请设计实验或活动验证地理读图技巧。',
    tags: ['地理读图技巧'],
    knowledgePointIds: ['m-geo-004'], subject: '地理', grade: 'middle',
  },

];
