...
| 工具 | 默认行为 | 说明 |
|------|---------|------|
| `read` / `grep` / `find` / `ls` | 自动执行 | 只读，无风险 |
| `bash` | 自动执行 | 可执行 shell 命令 |
| `write` / `edit` | 自动执行 | 修改仓库文件 |

> 迁移到 SDK 后，工具由 `AgentSession` 自动执行、无逐条确认（与原 `pi --print`
> 非交互模式行为一致）。原设计文档里的「需确认」交互从未在代码中落地（`toolConfirm`
> 是死代码，已删除）。信任边界：请仅在信任的项目目录下运行。
