...
## 方案 2 修订记录（2026-09-06 实施）

**用户决策**：注册免邮箱验证（Cloudflare 原生免费通道不存在任意发信；MailChannels 付费，故砍掉注册环节验证码）。

**与本计划的差异（已实现，以本节为准）**：

| 计划描述 | 实际实现 |
|---|---|
| 注册走验证码验证：register → verify 两步 | 注册即登录：`register` 直接签发 `{ user, tokens }`、`email_verified=1`、不发任何邮件 |
| 后端 `verify`、`resend` 端点 | 已删除（`worker/routes/auth.ts`） |
| `AuthContext.verify` / `resendCode` / `AuthModal` 验证码 / `EMAIL_NOT_VERIFIED` 分支 | 已删除；`register` 返回 `Promise<void>` |
| `AuthModal` 五视图（login/register/verify/forgot/reset） | 四视图（login/register/forgot/reset），注册成功直接 `onClose()` |
| 错误码映射 `EMAIL_NOT_VERIFIED` / `INVALID_CODE` / `CODE_EXPIRED` / resend 404 | 已移除；`forgot`/`reset` 错误文案保留 |
| 接口契约 `POST /api/auth/verify` / `resend` | 已移除；`register` 响应改为 `{ user, tokens }` |

**验证**：`tsc` ✅ / `eslint` 零错误 ✅ / `vite build` ✅ / 进程内 Worker E2E（mock D1/KV）13/13 ✅。

**保留**：`forgot`/`reset` 密码找回链路（走 `worker/lib/email.ts` MailChannels 发信；通道未激活时由 `FROM_EMAIL` 未配置兜底 console.log 降级）。

**安全性备注**：邮箱免验证后安全性依赖密码强度（≥8 位）与找回通道；如需恢复入驻验证，在后端 `handleAuth` 的 register 分支恢复 `email_verifications` + `sendVerificationEmail` 逻辑并补回 `verify` 端点。
